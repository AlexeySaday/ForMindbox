...
namespace Calc
{
    ...
    public static class Fabrica// Здесь я решил реализовать паттерн фабрика
    {
        static IMathArea area;
        public static double AddingFigure(string name, params double[] side)
        {
            switch (name)
            {
                case "Triangle":
                ...
                case "Circle":
                    ...
                    break;
                //case "Rectangle":
                //    area = new Rectangle(side[0], side[1]); // Добавить фигуру очень просто, что делает программу более юзабельным для пользователя
                //    break;
                default:
                    throw new Exception("Нет данной фигуры"); 
            }
            return area.Area();
        }
    }
